...
## Example:  
##          Rscript  A53Pups.R     1-covFiles/A-rmXY     2-A53Pups/A-rmXY   >     2-A53Pups_A-rmXY.runLog  2>&1     
...
Sex_NC_g = c()
Files_NC_g <- c(
  paste( inputDir_g,   "A53_69_A53C-boy-X_Rep1.CpG.bismark.cov",      sep="/"),
  paste( inputDir_g,   "A53_69_A53D-boy-X_Rep1.CpG.bismark.cov",      sep="/")
)
 
if( length(Files_NC_g) >= 1) {
    mySampleID_NC_g <- rep( x="batch1", times=length(Files_NC_g) )
...
Sex_IVF_fresh_g = c()
Files_IVF_fresh_g <- c(   
  paste( inputDir_g,     "A53_69_A53C-boy-X_Rep5.CpG.bismark.cov",   sep="/"),
  paste( inputDir_g,     "A53_69_A53D-boy-X_Rep5.CpG.bismark.cov",     sep="/")  
)
 
if( length(Files_IVF_fresh_g) >= 1) {
    mySampleID_IVF_fresh_g <- rep( x="batch2", times=length(Files_IVF_fresh_g) )
...
Sex_IVF_frozen_g = c()
Files_IVF_frozen_g <- c(  
  paste( inputDir_g,     "A53_69_A53C_Rep6.CpG.bismark.cov",   sep="/"),
  paste( inputDir_g,     "A53_69_A53D-boy-X_Rep6.CpG.bismark.cov",     sep="/")  
)
 
if( length(Files_IVF_frozen_g) >= 1) {
    mySampleID_IVF_frozen_g <- rep( x="batch3", times=length(Files_IVF_frozen_g) )
...
Sex_ICSI_fresh_g = c()
Files_ICSI_fresh_g <- c(
  paste( inputDir_g,     "chenwei2_69_A53D-boy-X_Rep1.bismark.cov",   sep="/")
)
  
if( length(Files_ICSI_fresh_g) >= 1) {
    mySampleID_ICSI_fresh_g <- rep( x="batch4", times=length(Files_ICSI_fresh_g) )
    Tech_ICSI_fresh_g <- mySampleID_ICSI_fresh_g
    for(i in c(1:length(mySampleID_ICSI_fresh_g)) ) {
      mySampleID_ICSI_fresh_g[i] = paste(mySampleID_ICSI_fresh_g[i], i+1, sep="_")
    }
...
if( length(Files_ICSI_frozen_g) >= 1) {
    mySampleID_ICSI_frozen_g <- rep( x="batch5", times=length(Files_ICSI_frozen_g) )
...
myTech_All_color_g = c( "batch1"="black", "batch2"="blue", "batch3"="green",  "batch4"="red", "batch5"="purple" )
...
MyTech_color_g <- function(  myTech_vector  ) {
  ...
  for(i in c(1:length(myTech_color)) ) {
    if(myTech_color[i] == "batch1")   { myTech_color[i] = c("batch1"="black")   }
    if(myTech_color[i] == "batch2")   { myTech_color[i] = c("batch2"="blue")    }
    if(myTech_color[i] == "batch3")   { myTech_color[i] = c("batch3"="green")   }
    if(myTech_color[i] == "batch4")   { myTech_color[i] = c("batch4"="red")     }
    if(myTech_color[i] == "batch5")   { myTech_color[i] = c("batch5"="purple")  }
  }
...
MyTech_number_g <- function(  myTech_vector  ) {
  myTech_color  = myTech_vector
  for(i in c(1:length(myTech_color)) ) {
    if(myTech_color[i] == "batch1")   { myTech_color[i] = 1 }
    if(myTech_color[i] == "batch2")   { myTech_color[i] = 2 }
    if(myTech_color[i] == "batch3")   { myTech_color[i] = 3 }
    if(myTech_color[i] == "batch4")   { myTech_color[i] = 4 }
    if(myTech_color[i] == "batch5")   { myTech_color[i] = 5 }
  }
  return(myTech_color)
}
...
myMainFunction_1_g  <- function(  myobj_temp1,   path_temp1,   binSize_temp1, binBases_temp1, dataFrame_temp1  ) {
  ...
  tryCatch(
      MyCluster_3_g(  mymeth2=meth_2two ,  path2=path_temp1_sub2,     file2="HierarchicalClustering_byMethylKit_",   width2=4,   height2=3 ),
      error = function(err){"000"}
  )
  ...
  tryCatch(
        myHierarchicalClustering_1_g(  mat_3three=mat_2two,   path_temp1=path_temp1_sub7,   width1=3, height1=3  ) ,
        error = function(err){"000"}
  )
...
gene.obj_total_3kb_g     = readTranscriptFeatures(myRefSeqGenes_g, remove.unusual=TRUE, up.flank=2000, down.flank=1000,  unique.prom=TRUE)
...
##################################################################################################################
  ...
  myBinSize_g = 300
  myMinimumCs_g = 3
  myMinimumCs_speRegions_g = 1
  
  
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/1_NC-vs-IVFfresh",  sep="") ,   
  ...
  myMainFunction_speRegions_All_g(  myobj_temp2 = myobj_nor_g,   
                                    path_temp2 = paste(outDir_g, "/1_NC-vs-IVFfresh/SomeSpeRegions",  sep="") ,  
