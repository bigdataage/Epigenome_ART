## Load up the database and query regions
library(LOLA)
library(GenomicRanges)
library(ggplot2) 


## example: Rscript  LOLA-oneFile.R    Test/aa1.bed     Test/matrix1.bed    aa1  
...
myBedFile        = args[1]    ## target bed file
myBackgroundFile = args[2]    ## background bed file
myOutDir         = args[3]    ## output path
if( ! file.exists(myOutDir) ) { dir.create(myOutDir, recursive = TRUE) }


regionSetA = readBed(myBedFile)
myBackground  = readBed(myBackgroundFile)
regionDB   = loadRegionDB("LOLACore/hg38")


## Running the enrichment
locResults = runLOLA(userSets=regionSetA, userUniverse=myBackground, regionDB=regionDB, 
                     minOverlap = 1, cores = 4, redefineUserSets = FALSE, direction = "enrichment")
...
## Exploring results
writeCombinedEnrichment(locResults, outFolder=myOutDir ,  includeSplits=TRUE)

myOddsRatio=locResults$oddsRatio
mySupport=locResults$support
myLog10P = locResults$pValueLog
myCollection = locResults$collection
myFilename = locResults$filename
myFilename

myText <- paste(myCollection, myFilename, sep="_") 
length(myLog10P)
length(myCollection)
length(myFilename)
length(myText)

myText = sub(pattern=".bed", replacement="", x=myText, ignore.case = FALSE, perl = FALSE, fixed = FALSE, useBytes = FALSE)  
for( i in c(1:length(myText)) ) {
    print(i)
    myTemp = reverse(myText[i])
    if(nchar(myTemp)>50) {myTemp = substr(myTemp, 1, 50)}
    myText[i] = reverse( myTemp )
}
...
myForColour = myOddsRatio[1:20]
myForColour[myForColour >= 50] = 50
myMidValue <-  median(myForColour)

myDataFrame = data.frame(myXaxis = myLog10P[1:20], myYaxis=  rev( factor(myText[1:20], levels=unique(myText[1:20])) ), 
                         Support=mySupport[1:20], OddsRatio=myForColour )


FigureTemp1 <- ggplot( data = myDataFrame, aes(x = myXaxis, y = myYaxis, size=Support , colour=OddsRatio ) ) + 
  geom_point( ) +    scale_colour_gradientn(colours = c("blue", "yellow2", "red"), values= c(0, 0.2, 1) ) +
  scale_y_discrete( breaks=  myDataFrame$myYaxis , labels=  rev( myDataFrame$myYaxis )   ) + 
  xlab("-log10(p-value)") +   ylab("Genomic Regions") +   ggtitle("Enrichment of Genomic Ranges") +  theme_bw()
MySaveGgplot2_1( path1=myOutDir, fileName1="Top20", height1=6, width1=8)
...
myForColour = myOddsRatio[1:30]
myForColour[myForColour >= 50] = 50
myMidValue <-  median(myForColour)

myDataFrame = data.frame(myXaxis = myLog10P[1:30], myYaxis=  rev( factor(myText[1:30], levels=unique(myText[1:30])) ), 
                         Support=mySupport[1:30], OddsRatio=myForColour )
...
FigureTemp1 <- ggplot( data = myDataFrame, aes(x = myXaxis, y = myYaxis, size=Support , colour=OddsRatio ) ) + 
  ...
  scale_y_discrete( breaks=  myDataFrame$myYaxis , labels=  rev( myDataFrame$myYaxis )   ) + 
  xlab("-log10(p-value)") +   ylab("Genomic Regions") +   ggtitle("Enrichment of Genomic Ranges") +  theme_bw()
MySaveGgplot2_1( path1=myOutDir, fileName1="Top30", height1=8, width1=8)
...
myForColour = myOddsRatio[1:40]
myForColour[myForColour >= 50] = 50
myMidValue <-  median(myForColour)

myDataFrame = data.frame(myXaxis = myLog10P[1:40], myYaxis=  rev( factor(myText[1:40], levels=unique(myText[1:40])) ), 
                         Support=mySupport[1:40], OddsRatio=myForColour )
...
FigureTemp1 <- ggplot( data = myDataFrame, aes(x = myXaxis, y = myYaxis, size=Support , colour=OddsRatio ) ) + 
  ...
  scale_y_discrete( breaks=  myDataFrame$myYaxis , labels=  rev( myDataFrame$myYaxis )   ) + 
  xlab("-log10(p-value)") +   ylab("Genomic Regions") +   ggtitle("Enrichment of Genomic Ranges") +  theme_bw()
MySaveGgplot2_1( path1=myOutDir, fileName1="Top40", height1=10, width1=8)
