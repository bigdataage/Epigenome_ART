...
###########################################################################################################################################################################
rawMatrix_1 <- read.table("diff2_number.txt", header=FALSE,   sep="\t" )  
dim(rawMatrix_1 )
rawMatrix_1[1:3,]
...
matrix1 = rawMatrix_1[2:11, 3:4]
colName1 = as.vector(unlist( rawMatrix_1[1, 3:4] ))
rowName1a = as.vector(unlist( rawMatrix_1[2:11, 1] ))
rowName1b = as.vector(unlist( rawMatrix_1[2:11, 2] ))
...
DataFrame_Local1 <- data.frame( xAxis = rep( colName1 , each = nrow(matrix1) ) , 
                               yAxis  = as.numeric( as.character ( unlist( matrix1 )) ), 
                               SampleType1 = rep( rowName1a , ncol(matrix1) ),
                               SampleType2 = rep( rowName1b , ncol(matrix1) )
)
...
FigureTemp1 <- ggplot(DataFrame_Local1, aes(x= SampleType1 , fill=SampleType2, y=yAxis ) )  +
  geom_bar( stat="identity"    ) +  facet_grid(. ~ xAxis) +
  xlab("Samples") + ylab("Number of DMCs") +  MyTheme_1(textSize=12 )   
MySaveGgplot2_1(ggplot2Figure1=FigureTemp1,  path1=AllResults_g , fileName1="diff2-1-a",  height1=4,  width1=6)

 


FigureTemp1 <- ggplot(DataFrame_Local1, aes(x=xAxis  , fill=SampleType2, y=yAxis ) )  +
  geom_bar( stat="identity"    ) +  facet_grid(. ~ SampleType1) +
  xlab("Samples") + ylab("Number of DMCs") +  MyTheme_1(textSize=12 )   
MySaveGgplot2_1(ggplot2Figure1=FigureTemp1,  path1=AllResults_g , fileName1="diff2-1-b",  height1=4,  width1=6)
