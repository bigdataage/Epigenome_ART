...
## example: Rscript  myChIPseeker.hg38.R    inputDir    outDir  
######################################################################################################################################################
...
inputDir   = args[1];     ## files of the same group in this folder (input dir)
outPath    = args[2];     ## output file path
# inputDir = "2-formated/group1"
# outPath  = "3-annotation/group1" 
...
MyPeaksDistribution_OneGroup_1_g <- function(myPeak_t1, myPath_t1, halfSize=3000) { 
  ...
  ## ------------------------------------------------------------------------
  peakAnno <- annotatePeak( myPeak_t1, tssRegion=c(-halfSize, halfSize),  TxDb=my_txdb_g, annoDb="org.Hs.eg.db"  )  
...
MyGeneOntology_OneGroup_1_g <- function(MyMaxDis=900000000,  MyPeaksAnno, MyFolder,  MyFileName ) {
  ...
  bp1 <- enrichGO(as.data.frame(peakAnno2)$geneId,  OrgDb='org.Hs.eg.db',  ont="BP",  readable=TRUE)
  ...
  #####################################################################
  MF1 <- enrichGO(as.data.frame(peakAnno2)$geneId,  OrgDb='org.Hs.eg.db',  ont="MF",  readable=TRUE)
  ...
  #####################################################################################################
  CC1 <- enrichGO(as.data.frame(peakAnno2)$geneId,  OrgDb='org.Hs.eg.db',  ont="CC",  readable=TRUE)
  ...
  #####################################################################################################
  Reactome1 <- enrichPathway(gene=as.data.frame(peakAnno2)$geneId, organism = "human")   ## enrichPathway {ReactomePA}
...
MyKEGG_OneGroup_1_g <- function(MyMaxDis=900000000,  MyPeaksAnno, MyFolder,  MyFileName ) {
  ...
  enrich_KEGG <- enrichKEGG(gene=as.data.frame(peakAnno2)$geneId, organism="hsa", keyType="kegg",  pvalueCutoff=0.05)
  ...
  pdf(file=paste(MyFolder, "/", MyFileName, "_2_KEGG_enrichment.pdf", sep=""),  width=12, height=10)
    print( barplot(enrich_KEGG, drop=TRUE, showCategory=20, x = "Count",     colorBy = "p.adjust", font.size = 12, title = "")  )
    print( barplot(enrich_KEGG, drop=TRUE, showCategory=20, x = "GeneRatio", colorBy = "p.adjust", font.size = 12, title = "")  )
    print( dotplot(enrich_KEGG, x="geneRatio", colorBy="p.adjust", showCategory=20, split=NULL, font.size=12, title="")  )
    print( dotplot(enrich_KEGG, x="count",     colorBy="p.adjust", showCategory=20, split=NULL, font.size=12, title="")  )
    print( enrichMap(enrich_KEGG, n = 10, fixed = TRUE, vertex.label.font = 1)  )
    print( enrichMap(enrich_KEGG, n = 20, fixed = TRUE, vertex.label.font = 1)  )
...
Disease_OneGroup_1_g <- function(MyMaxDis=900000000,  MyPeaksAnno, MyFolder,  MyFileName ) {
  ...
  pdf(file=paste(MyFolder, "/", MyFileName, "_2_DO_enrichment.pdf", sep=""),  width=12, height=10)
      print( barplot(enrich_DO, drop=TRUE, showCategory=20, x = "Count",     colorBy = "p.adjust", font.size = 12, title = "") )
      print( barplot(enrich_DO, drop=TRUE, showCategory=20, x = "GeneRatio", colorBy = "p.adjust", font.size = 12, title = "") )
      print( dotplot(enrich_DO, x="geneRatio", colorBy="p.adjust", showCategory=20, split=NULL, font.size=12, title="") )
      print( dotplot(enrich_DO, x="count",     colorBy="p.adjust", showCategory=20, split=NULL, font.size=12, title="") )
      print( enrichMap(enrich_DO, n = 10, fixed = TRUE, vertex.label.font = 1) )
      print( enrichMap(enrich_DO, n = 20, fixed = TRUE, vertex.label.font = 1) )
...
MyAnnotation_OneGroup_1_g <- function(myFile_t1, myPath_t1) {
  ...
  peakAnno <- annotatePeak( myPeak_t1, tssRegion=c(-1000, 1000),  TxDb=my_txdb_g, annoDb="org.Hs.eg.db"  )  
