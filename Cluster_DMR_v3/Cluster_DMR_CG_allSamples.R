...
## Example:  
## Rscript  Cluster_DMR_CG_allSamples.R     2_rmXY_CpG_5Reads     5A_All_rmXY_1kb          
...
# inputDir_g =  "2_rmXY_CpG_5Reads"
# outDir_g   =  "5A_All_rmXY_1kb "
print(inputDir_g)   
print(outDir_g)
...
Files_NC_g <- c(
  paste( inputDir_g,   "73_NC-E114-C-Boy-merge_Rep1.bismark.cov", sep="/" ),
  paste( inputDir_g,   "74_E54-NC-D_Rep3.bismark.cov",            sep="/" ),
  ...
  paste( inputDir_g,   "73_NC-E114-D-Girl_Rep1.bismark.cov",      sep="/"),
  paste( inputDir_g,   "74_E54-NC-C_Rep3.bismark.cov",            sep="/"),
  paste( inputDir_g,   "75_E98-NC-C_Rep3.bismark.cov",            sep="/"),
  paste( inputDir_g,   "61_NC-BS2-C-Girl-merge_Rep1.bismark.cov", sep="/"),
  paste( inputDir_g,   "61_NC-BS2-D-Girl-merge_Rep1.bismark.cov", sep="/"),
  paste( inputDir_g,   "63_NC-E8-C-Girl_Rep3.bismark.cov",        sep="/"),
...
Files_NC_g_parents <- c(
  paste( inputDir_g,   "73_NC-E114-F-Father-merge_Rep1.bismark.cov",   sep="/"),
  paste( inputDir_g,   "74_NC-E54-F-Father_Rep3.bismark.cov",          sep="/"),
  paste( inputDir_g,   "75_E98-NC-F_Rep3.bismark.cov",                 sep="/"),
  paste( inputDir_g,   "61_NC-BS2-F-Father-merge_Rep1.bismark.cov",    sep="/"),
  paste( inputDir_g,   "63_NC-E8-F-Father_Rep2.bismark.cov",           sep="/"), 
  ...
  paste( inputDir_g,   "27_E16F-father-NC_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,   "40_E12F-father-NC_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,   "41_E13F-father-NC_Rep1.bismark.cov",           sep="/"),
  
  #paste( inputDir_g,   "73_NC-E114-M-Mother-merge_Rep1.bismark.cov",     sep="/"),
  paste( inputDir_g,   "74_E54-NC-M_Rep3.bismark.cov",                   sep="/"),
  paste( inputDir_g,   "75_E98-NC-M_Rep3.bismark.cov",                   sep="/"), 
  paste( inputDir_g,   "61_NC-BS2-M-Mother-merge_Rep1.bismark.cov",      sep="/"),
  paste( inputDir_g,   "63_NC-E8-M-Mother-merge_Rep1.bismark.cov",       sep="/"),
  ...
  paste( inputDir_g,   "27_E16M-mother-NC-merge_Rep1.bismark.cov",       sep="/"),
  paste( inputDir_g,   "40_E12M-mother-NC_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,   "41_E13M-mother-NC_Rep1.bismark.cov",             sep="/")   
)

mySampleID_NC_g_parents <- rep( x="NC", times=length(Files_NC_g_parents) )
Tech_NC_g_parents <- mySampleID_NC_g_parents
  
for(i in c(1:length(mySampleID_NC_g_parents)) ) {
  mySampleID_NC_g_parents[i] = paste(mySampleID_NC_g_parents[i], i, sep="_")
}

myTreatment_NC_g_parents <- rep( x=0,  times=length(Files_NC_g_parents) )

Sex_NC_g_parents = rep( x="father",  times=length(Files_NC_g_parents) )  
for(i in c(1:length(Sex_NC_g_parents)) ) {
  Sex_NC_g_parents[i] = "father"
  if(i>=12) { Sex_NC_g_parents[i] = "mother" }
}

  

Files_IVF_fresh_g_parents <- c(
  paste( inputDir_g,    "76_ART-E18-F-Father_Rep3.bismark.cov",              sep="/"),
  paste( inputDir_g,    "77_ART-E69-F-Father_Rep1.bismark.cov",              sep="/"),
  paste( inputDir_g,    "78_ART-E101-F-Father_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "79_ART-E72-F-Father_Rep3.bismark.cov",              sep="/"),
  paste( inputDir_g,    "64_ART-BS18-F-Father_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "65_ART-BS29-F-Father_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "66_ART-E29-F-Father_Rep2.bismark.cov",              sep="/"),  
  ...
  paste( inputDir_g,    "28_E23-IVF-fresh-F_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "29_W76-IVF-fresh-F_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "30_W1452-IVF-fresh-F_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "42_W53-IVF-fresh-F_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "43_W81-IVF-fresh-F_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "44_W1694F-father-IVF-fresh-merge_Rep1.bismark.cov", sep="/"),
  
  paste( inputDir_g,    "76_ART-E18-M-Mother_Rep3.bismark.cov",          sep="/"),
  paste( inputDir_g,    "77_ART-E69-M-Mother_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "78_ART-E101-M-Mother_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "79_E72-ART-M_Rep3.bismark.cov",                 sep="/"),
  paste( inputDir_g,    "64_ART-BS18-M-Mother-merge_Rep1.bismark.cov",   sep="/"),
  paste( inputDir_g,    "65_ART-BS29-M-Mother_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "66_ART-E29-M-Mother-merge_Rep1.bismark.cov",    sep="/"),
  ...
  paste( inputDir_g,    "28_E23-IVF-fresh-M-merge_Rep1.bismark.cov",     sep="/"),
  paste( inputDir_g,    "29_W76-IVF-fresh-M_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "30_W1452-IVF-fresh-M_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "42_W53-IVF-fresh-M_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "43_W81-IVF-fresh-M_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "44_W1694M-mother-IVF-fresh_Rep1.bismark.cov",   sep="/")  
)

mySampleID_IVF_fresh_g_parents <- rep( x="IVF_fresh", times=length(Files_IVF_fresh_g_parents) )
Tech_IVF_fresh_g_parents <- mySampleID_IVF_fresh_g_parents

for(i in c(1:length(mySampleID_IVF_fresh_g_parents)) ) {
  mySampleID_IVF_fresh_g_parents[i] = paste(mySampleID_IVF_fresh_g_parents[i], i, sep="_")
}

myTreatment_IVF_fresh_g_parents <- rep( x=1,  times=length(Files_IVF_fresh_g_parents) )

Sex_IVF_fresh_g_parents = rep( x="father",      times=length(Files_IVF_fresh_g_parents) ) 
for(i in c(1:length(Sex_IVF_fresh_g_parents)) ) {
  Sex_IVF_fresh_g_parents[i] = "father"
  if(i>=17) { Sex_IVF_fresh_g_parents[i] = "mother" }
}



Files_IVF_frozen_g_parents <- c(
  paste( inputDir_g,    "16_W839F-father-IVF-frozen_Rep1.bismark.cov",          sep="/"), 
  paste( inputDir_g,    "17_W1524F-father-IVF-frozen_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "18_W1387F-IVF-frozen_Rep1.bismark.cov",                sep="/"),
  paste( inputDir_g,    "19_W1458F-IVF-frozen_Rep1.bismark.cov",                sep="/"), 
  paste( inputDir_g,    "1_A42F-father-IVF-frozen_Rep1.bismark.cov",            sep="/"),
  paste( inputDir_g,    "2_W1785F-father-IVF-frozen_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "3_Q8-W811F-father-IVF-frozen_Rep1.bismark.cov",        sep="/"),
  paste( inputDir_g,    "4_W28F-father-IVF-frozen_Rep1.bismark.cov",            sep="/"),
  ...
  paste( inputDir_g,    "31_W1149-IVF-frozen-F_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "32_W88-IVF-frozen-F_Rep1.bismark.cov",                 sep="/"),
  paste( inputDir_g,    "33_W104-IVF-frozen-F_Rep1.bismark.cov",                sep="/"),
  paste( inputDir_g,    "45_E14F-father-IVF-frozen_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "46_W857F-father-IVF-frozen_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "47_E32F-father-IVF-frozen_Rep1.bismark.cov",           sep="/"),
  
  paste( inputDir_g,    "16_W839M-Mother-IVF-frozen_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "17_Q21-W1524M-IVF-frozen_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "18_W1387M-IVF-frozen_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "19_W1458M-IVF-frozen_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "1_A42M-Mother-IVF-frozen_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "2_Q16-W1785M-mother-IVF-frozen_Rep1.bismark.cov",     sep="/"),
  paste( inputDir_g,    "3_Q2-W811M-mother-IVF-frozen_Rep1.bismark.cov",       sep="/"),
  paste( inputDir_g,    "4_W28M-Mother-IVF-frozen_Rep1.bismark.cov",           sep="/"),
  ...
  paste( inputDir_g,    "31_W1149-IVF-frozen-M_Rep1.bismark.cov",              sep="/"),
  paste( inputDir_g,    "32_W88-IVF-frozen-M_Rep1.bismark.cov",                sep="/"),
  #paste( inputDir_g,    "33_W104-IVF-frozen-M_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "45_E14M-mother-IVF-frozen-merge_Rep1.bismark.cov",    sep="/"),
  paste( inputDir_g,    "46_W857M-mother-IVF-frozen_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "47_E32M-mother-IVF-frozen_Rep1.bismark.cov",          sep="/")
)

mySampleID_IVF_frozen_g_parents <- rep( x="IVF_frozen", times=length(Files_IVF_frozen_g_parents) )
Tech_IVF_frozen_g_parents <- mySampleID_IVF_frozen_g_parents

for(i in c(1:length(mySampleID_IVF_frozen_g_parents)) ) {
  mySampleID_IVF_frozen_g_parents[i] = paste(mySampleID_IVF_frozen_g_parents[i], i, sep="_")
}

myTreatment_IVF_frozen_g_parents <- rep( x=2,  times=length(Files_IVF_frozen_g_parents) )

Sex_IVF_frozen_g_parents = rep( x="father",      times=length(Files_IVF_frozen_g_parents) )
for(i in c(1:length(Sex_IVF_frozen_g_parents)) ) {
  Sex_IVF_frozen_g_parents[i] = "father"
  if(i>=18) { Sex_IVF_frozen_g_parents[i] = "mother" }
}



Files_ICSI_fresh_g_parents <- c(
  paste( inputDir_g,    "20_W1191F-ICSI-fresh_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "21_Q20-W1507F-ICSI-fresh_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "22_W1636F-ICSI-fresh_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "5_E95F-father-ICSI-fresh_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "6_W655F-father-ICSI-fresh_Rep1.bismark.cov",        sep="/"),
  paste( inputDir_g,    "7_W1276F-father-ICSI-fresh_Rep1.bismark.cov",       sep="/"),
  ...
  paste( inputDir_g,    "34_W200-ICSI-fresh-F_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "35_W766F-ICSI-fresh_Rep1.bismark.cov",              sep="/"),
  paste( inputDir_g,    "36_W928-ICSI-fresh-F_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "48_W808F-father-ICSI-fresh_Rep1.bismark.cov",       sep="/"),
  paste( inputDir_g,    "49_W924-ICSI-fresh-F_Rep1.bismark.cov",             sep="/"),
  paste( inputDir_g,    "50_W934F-father-ICSI-fresh_Rep1.bismark.cov",       sep="/"),
  
  paste( inputDir_g,    "20_W1191M-ICSI-fresh_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "21_W1507M-ICSI-fresh_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "22_W1636M-ICSI-fresh_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "5_E95M-Mother-ICSI-fresh_Rep1.bismark.cov",           sep="/"),
  paste( inputDir_g,    "7_W1276M-mother-ICSI-fresh_Rep1.bismark.cov",         sep="/"),
  ...
  paste( inputDir_g,    "34_W200-ICSI-fresh-M_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "35_W766M-ICSI-fresh_Rep1.bismark.cov",                sep="/"),
  paste( inputDir_g,    "48_W808M-mother-ICSI-fresh_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "49_W924-ICSI-fresh-M_Rep1.bismark.cov",               sep="/") 
)

mySampleID_ICSI_fresh_g_parents <- rep( x="ICSI_fresh", times=length(Files_ICSI_fresh_g_parents) )
Tech_ICSI_fresh_g_parents <- mySampleID_ICSI_fresh_g_parents

for(i in c(1:length(mySampleID_ICSI_fresh_g_parents)) ) {
  mySampleID_ICSI_fresh_g_parents[i] = paste(mySampleID_ICSI_fresh_g_parents[i], i, sep="_")
}

myTreatment_ICSI_fresh_g_parents <- rep( x=3,  times=length(Files_ICSI_fresh_g_parents) )

Sex_ICSI_fresh_g_parents = rep( x="father",      times=length(Files_ICSI_fresh_g_parents) )  
for(i in c(1:length(Sex_ICSI_fresh_g_parents)) ) {
  Sex_ICSI_fresh_g_parents[i] = "father"
  if(i>=16) { Sex_ICSI_fresh_g_parents[i] = "mother" }
}



Files_ICSI_frozen_g_parents <- c(
  paste( inputDir_g,    "23_E45F-father-ICSI-frozen_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "24_W774F-ICSI-frozen_Rep1.bismark.cov",                sep="/"),
  paste( inputDir_g,    "25_E115F-father-ICSI-frozen_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "26_A19F-ICSI-frozen_Rep1.bismark.cov",                 sep="/"),
  paste( inputDir_g,    "8_W845F-father-ICSI-frozen_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "15_Q7-W871F-father-ICSI-frozen_Rep1.bismark.cov",      sep="/"),
  paste( inputDir_g,    "38_W1531-ICSI-frozen-F_Rep1.bismark.cov",              sep="/"),
  paste( inputDir_g,    "39_W1656F-father-ICSI-frozen_Rep1.bismark.cov",        sep="/"),
  paste( inputDir_g,    "51_W1699F-father-ICSI-frozen_Rep1.bismark.cov",        sep="/"),
  paste( inputDir_g,    "52_W39-ICSI-frozen-F_Rep1.bismark.cov",                sep="/"),
  
  #paste( inputDir_g,    "23_E45M-Mother-ICSI-frozen_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "24_W774M-ICSI-frozen_Rep1.bismark.cov",                sep="/"),
  paste( inputDir_g,    "25_E115M-mother-ICSI-frozen_Rep1.bismark.cov",         sep="/"),
  paste( inputDir_g,    "26_A19M-ICSI-frozen_Rep1.bismark.cov",                 sep="/"),  
  paste( inputDir_g,    "8_W845M-Mother-ICSI-frozen_Rep1.bismark.cov",          sep="/"),
  paste( inputDir_g,    "15_Q10-W871M-mother-ICSI-frozen_Rep1.bismark.cov",     sep="/"),  
  #paste( inputDir_g,    "37_W618-ICSI-frozen-M_Rep1.bismark.cov",               sep="/"),
  paste( inputDir_g,    "38_W1531-ICSI-frozen-M_Rep1.bismark.cov",              sep="/"),
  paste( inputDir_g,    "39_W1656M-mother-ICSI-frozen_Rep1.bismark.cov",        sep="/"),
  paste( inputDir_g,    "51_W1699M-mother-ICSI-frozen_Rep1.bismark.cov",        sep="/"),
  paste( inputDir_g,    "52_W39-ICSI-frozen-M_Rep1.bismark.cov",                sep="/")  
)

mySampleID_ICSI_frozen_g_parents <- rep( x="ICSI_frozen", times=length(Files_ICSI_frozen_g_parents) )
Tech_ICSI_frozen_g_parents <- mySampleID_ICSI_frozen_g_parents

for(i in c(1:length(mySampleID_ICSI_frozen_g_parents)) ) {
  mySampleID_ICSI_frozen_g_parents[i] = paste(mySampleID_ICSI_frozen_g_parents[i], i, sep="_")
}

myTreatment_ICSI_frozen_g_parents <- rep( x=4,  times=length(Files_ICSI_frozen_g_parents) )

Sex_ICSI_frozen_g_parents = rep( x="father",      times=length(Files_ICSI_frozen_g_parents) )  
for(i in c(1:length(Sex_ICSI_frozen_g_parents)) ) {
  Sex_ICSI_frozen_g_parents[i] = "father"
  if(i>=11) { Sex_ICSI_frozen_g_parents[i] = "mother" }
}
...
Files_All_vector_g_parents <- c(
  Files_NC_g_parents,
  Files_IVF_fresh_g_parents,
  Files_IVF_frozen_g_parents,
  Files_ICSI_fresh_g_parents,
  Files_ICSI_frozen_g_parents  
)
Files_All_list_g_parents <- as.list( Files_All_vector_g_parents )


mySampleID_All_vector_g_parents <- c( 
  mySampleID_NC_g_parents,
  mySampleID_IVF_fresh_g_parents,
  mySampleID_IVF_frozen_g_parents,
  mySampleID_ICSI_fresh_g_parents,
  mySampleID_ICSI_frozen_g_parents  
)
mySampleID_All_list_g_parents <- as.list( mySampleID_All_vector_g_parents )


myTreatment_All_vector_g_parents <- c( 
  myTreatment_NC_g_parents,
  myTreatment_IVF_fresh_g_parents,
  myTreatment_IVF_frozen_g_parents,
  myTreatment_ICSI_fresh_g_parents,
  myTreatment_ICSI_frozen_g_parents 
)       
myTreatment_All_list_g_parents <- as.list( myTreatment_All_vector_g_parents )


mySex_All_vector_g_parents <- c( 
  Sex_NC_g_parents,
  Sex_IVF_fresh_g_parents,
  Sex_IVF_frozen_g_parents,
  Sex_ICSI_fresh_g_parents,
  Sex_ICSI_frozen_g_parents 
)       
mySex_All_list_g_parents <- as.list( mySex_All_vector_g_parents )


myTech_All_vector_g_parents <- c( 
  Tech_NC_g_parents,
  Tech_IVF_fresh_g_parents,
  Tech_IVF_frozen_g_parents,
  Tech_ICSI_fresh_g_parents,
  Tech_ICSI_frozen_g_parents 
)       
myTech_All_list_g_parents <- as.list( myTech_All_vector_g_parents )



length( Files_All_vector_g_parents )
length( Files_All_list_g_parents )
length( mySampleID_All_vector_g_parents )
length( mySampleID_All_list_g_parents )
length( myTreatment_All_vector_g_parents )
length( myTreatment_All_list_g_parents )
length( mySex_All_vector_g_parents )
length( mySex_All_list_g_parents )
length( myTech_All_vector_g_parents )
length( myTech_All_list_g_parents )

allSamples_info_g_parents = cbind(Files_All_vector_g_parents, mySampleID_All_vector_g_parents, myTreatment_All_vector_g_parents, mySex_All_vector_g_parents, myTech_All_vector_g_parents)

write.table(allSamples_info_g_parents , 
            file = paste(outDir_g,   "allSamples_info_parents.txt",  sep="/"), 
            append =FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", 
            row.names = FALSE,  col.names = TRUE, qmethod = c("escape", "double"),  fileEncoding = "")
...
myHierarchicalClustering_1_g  <- function(  mat_3three,   path_temp1,   dataFrame_temp1, width1, height1   )  {
...
myDiff_DMC_DMR_g <- function(  methobj2,   path2  ) {
  ...
  print( hist( myQvalue_2two_sub2[myQvalue_2two_sub2<0.01],  nclass=20, xlim=c(0, 0.01), freq=FALSE) )
  ...
  print( hist( myQvalue_2two_sub2[myQvalue_2two_sub2<0.01],  nclass=20, xlim=c(0, 0.01), freq=TRUE) )
  dev.off() 

  myQvalue_2two_sub2_log10 <- -log10(myQvalue_2two_sub2)
  pdf(paste(path2,  "1B_qvalue_distribution_log10.pdf",  sep="/"))
  print( hist( myQvalue_2two_sub2_log10[myQvalue_2two_sub2_log10<100],  nclass=100, xlim=c(0, 100), freq=FALSE) )
  print( hist( myQvalue_2two_sub2_log10[myQvalue_2two_sub2_log10<10],   nclass=100, xlim=c(0, 10),  freq=FALSE) )
  print( hist( myQvalue_2two_sub2_log10[myQvalue_2two_sub2_log10<2],    nclass=100, xlim=c(0, 2),   freq=FALSE) )
  print( hist( myQvalue_2two_sub2_log10[myQvalue_2two_sub2_log10<100],  nclass=100, xlim=c(0, 100), freq=TRUE) )
  print( hist( myQvalue_2two_sub2_log10[myQvalue_2two_sub2_log10<10],   nclass=100, xlim=c(0, 10),  freq=TRUE) )
  print( hist( myQvalue_2two_sub2_log10[myQvalue_2two_sub2_log10<2],    nclass=100, xlim=c(0, 2),   freq=TRUE) )
  dev.off() 
  ...
  print( hist(myMethDi_2two_sub2[myMethDi_2two_sub2<=30], nclass=100, xlim=c(0, 30),   freq=FALSE) )
  print( hist(myMethDi_2two_sub2[myMethDi_2two_sub2<=20], nclass=100, xlim=c(0, 20),   freq=FALSE) )
  ...
  print( hist(myMethDi_2two_sub2[myMethDi_2two_sub2<=30], nclass=100, xlim=c(0, 30),   freq=TRUE) )
  print( hist(myMethDi_2two_sub2[myMethDi_2two_sub2<=20], nclass=100, xlim=c(0, 20),   freq=TRUE) )
  dev.off() 
...
myMainFunction_1_g  <- function(  myobj_temp1,   path_temp1,   binSize_temp1, binBases_temp1, dataFrame_temp1  ) {
  ...
  MyCluster_3_g(  mymeth2=meth_2two ,  path2=path_temp1_sub2,     file2="HierarchicalClustering_byMethylKit_",   width2=30,   height2=12 )
  ...
  myHierarchicalClustering_1_g(  mat_3three=mat_2two,   path_temp1=path_temp1_sub7,   dataFrame_temp1=dataFrame_temp1 , width1=30, height1=12  )
  ...
  path_temp1_sub9 = paste(path_temp1, "9_calculateDiffMethDSS", sep="/")
  if( ! file.exists(path_temp1_sub9) ) { dir.create(path_temp1_sub9, recursive = TRUE) }
  myDiff_calculateDiffMethDSS_g(  methobj2=meth_2two,   path2=path_temp1_sub9  )
  ...
  path_temp1_sub11 = paste(path_temp1, "11_DMR_overdispersion_Ftest", sep="/")
  if( ! file.exists(path_temp1_sub11) ) { dir.create(path_temp1_sub11, recursive = TRUE) }
  myDiff_overdispersion_Ftest_g(  methobj2=meth_2two,   path2=path_temp1_sub11  )
...
##################################################################################################################
myOutDir_sub1_g = paste(outDir_g, "/0A_ReadRawFiles",  sep="") 
...
for( i in c(1:length(Files_All_vector_g)) ) {
  print(   dim(filtered.myobj_g[[i]])  )
}
sink()


myobj_nor_g <- filtered.myobj_g
##myobj_nor_g <- normalizeCoverage(filtered.myobj_g)
...
myOutDir_sub1_g2 = paste(outDir_g, "/0A_ReadRawFiles/removed",  sep="") 
...
for( i in c(1:length(Files_All_vector_g)) ) {
  print(   dim(removed.myobj_g[[i]])  )
}
sink()

sink( file=paste(myOutDir_sub1_g, "8_dimensions-all.txt", sep="/")  )
print("Raw  Kept  Removed")
for( i in c(1:length(Files_All_vector_g)) ) {
     print(   c( dim(myobj_g[[i]])[1], dim(filtered.myobj_g[[i]])[1], dim(removed.myobj_g[[i]])[1] ) )
}
sink()
...
## parents
##################################################################################################################
myOutDir_sub1_g_parents = paste(outDir_g, "/0B_ReadRawFiles_parents",  sep="") 
if( ! file.exists(myOutDir_sub1_g_parents) ) { dir.create(myOutDir_sub1_g_parents, recursive = TRUE) }

sink( file=paste(myOutDir_sub1_g_parents, "1_length-variables.txt", sep="/") )
length( Files_All_vector_g_parents )
length( Files_All_list_g_parents )
length( mySampleID_All_vector_g_parents )
length( mySampleID_All_list_g_parents )
...
length( myTech_All_vector_g_parents )
length( myTech_All_list_g_parents )
print( "#################### Files_All_vector_g_parents ####################" )
print( Files_All_vector_g_parents )
print( "#################### Files_All_list_g_parents ####################" )
print( Files_All_list_g_parents )
print( "#################### mySampleID_All_vector_g_parents ####################" )
print( mySampleID_All_vector_g_parents )
print( "#################### mySampleID_All_list_g_parents ####################" )
print( mySampleID_All_list_g_parents )
print( "#################### myTreatment_All_vector_g_parents ####################" )
print( myTreatment_All_vector_g_parents )
print( "#################### myTreatment_All_list_g_parents ####################" )
print( myTreatment_All_list_g_parents )
print( "#################### mySex_All_vector_g_parents ####################" )
print( mySex_All_vector_g_parents )
print( "#################### mySex_All_list_g_parents ####################" )
print( mySex_All_list_g_parents )
print( "#################### myTech_All_vector_g_parents ####################" )
print( myTech_All_vector_g_parents )
print( "#################### myTech_All_list_g_parents ####################" )
print( myTech_All_list_g_parents )
sink()


# read the files to a methylRawList object: myobj
sink( file=paste(myOutDir_sub1_g_parents, "2_theLog-of-read-inputFiles.txt", sep="/") )
myobj_g_parents = methRead( Files_All_list_g_parents,
               sample.id = mySampleID_All_list_g_parents,
               assembly  = "hg38",
               treatment = myTreatment_All_vector_g_parents,
               context   = "CpG",
               pipeline  = "bismarkCoverage",
               mincov    = lowestCoverage_g_parents,       ## >= n
               header    = FALSE
)
sink()


sink( file=paste(myOutDir_sub1_g_parents, "3_all-rawFiles.txt", sep="/") )
    print(Files_All_vector_g_parents)
    print("#########################")
    print(myobj_g_parents)
sink()


sink( file=paste(myOutDir_sub1_g_parents, "4_dimensions-of-eachFile-eachCov.txt", sep="/")  )
for( i in c(1:length(Files_All_vector_g_parents)) ) {
  print( "######################" )
  print(   Files_All_vector_g_parents[i]  )
  print(   dim(myobj_g_parents[[i]])  )
}
sink()


sink( file=paste(myOutDir_sub1_g_parents, "5_dimensions-of-eachCov.txt", sep="/")  )
for( i in c(1:length(Files_All_vector_g_parents)) ) {
  print(   dim(myobj_g_parents[[i]])  )
}
sink()
...
continue_on_error_g_parents <- function()  {
  print("NOTE: THERE WAS AN ERROR HERE. We are continuing because we have set 'options(error=continue_on_error())'")
}
# This is the key option
options(error=continue_on_error_g_parents) 




filtered.myobj_g_parents = filterByCoverage( myobj_g_parents,  lo.count = lowestCoverage_g_parents,  lo.perc = NULL,  hi.count = NULL,  hi.perc = 99.99,  chunk.size = 1e+06,  save.db = FALSE )

sink( file=paste(myOutDir_sub1_g_parents, "6_dimensions-of-eachCov.filtered.txt", sep="/")  )
for( i in c(1:length(Files_All_vector_g_parents)) ) {
  print(   dim(filtered.myobj_g_parents[[i]])  )
}
sink()


myobj_nor_g_parents <- filtered.myobj_g_parents
##myobj_nor_g_parents <- normalizeCoverage(filtered.myobj_g_parents)


removed.myobj_g_parents = filterByCoverage( myobj_g_parents,  lo.count = lowestCoverage_g_parents,  lo.perc = 99.99,  hi.count = NULL,  hi.perc = NULL,  chunk.size = 1e+06,  save.db = FALSE )
myOutDir_sub1_g_parents2 = paste(outDir_g_parents, "/0B_ReadRawFiles_parents/removed",  sep="") 
if( ! file.exists(myOutDir_sub1_g_parents2) ) { dir.create(myOutDir_sub1_g_parents2, recursive = TRUE) }

for( i in c(1:length(Files_All_vector_g_parents)) ) {
  file_name = Files_All_vector_g_parents[i]
  file_name = gsub(inputDir_g_parents, i, file_name)
  file_name = gsub("/", "__", file_name)
  write.table(getData(removed.myobj_g_parents[[i]])  , 
              file = paste(myOutDir_sub1_g_parents2,   file_name,  sep="/"), 
              append = FALSE, quote = FALSE, sep = "\t", eol = "\n", na = "NA", dec = ".", 
              row.names = FALSE,  col.names = TRUE, qmethod = c("escape", "double"),  fileEncoding = "")
}

sink( file=paste(myOutDir_sub1_g_parents, "7_dimensions-removed.txt", sep="/")  )
for( i in c(1:length(Files_All_vector_g_parents)) ) {
  print(   dim(removed.myobj_g_parents[[i]])  )
}
sink()

sink( file=paste(myOutDir_sub1_g_parents, "8_dimensions-all.txt", sep="/")  )
print("Raw  Kept  Removed")
for( i in c(1:length(Files_All_vector_g_parents)) ) {
     print(   c( dim(myobj_g_parents[[i]])[1], dim(filtered.myobj_g_parents[[i]])[1], dim(removed.myobj_g_parents[[i]])[1] ) )
}
sink()
...
pdf( file=paste(myOutDir_sub1_g_parents, "9A_MethylationStats-raw.pdf", sep="/")  )
for( i in c(1:length(myobj_g_parents)) ) {
  getMethylationStats(myobj_g_parents[[i]], plot=TRUE, both.strands=FALSE )
}
dev.off()
sink( file=paste(myOutDir_sub1_g_parents, "9B_MethylationStats-raw.txt", sep="/")  )
for( i in c(1:length(myobj_g_parents)) ) {
  print("##############################################")
  print( paste(Files_All_vector_g_parents[i],  ":", sep="") )
  print( getMethylationStats( myobj_g_parents[[i]] )  )
}
sink()
pdf( file=paste(myOutDir_sub1_g_parents, "10A_CoverageStats-raw.pdf", sep="/")  )
for( i in c(1:length(myobj_g_parents)) ) {
  getCoverageStats(myobj_g_parents[[i]], plot=TRUE, both.strands=FALSE )
}
dev.off()
sink( file=paste(myOutDir_sub1_g_parents, "10B_CoverageStats-raw.txt", sep="/")  )
for( i in c(1:length(myobj_g_parents)) ) {
  print("##############################################")
  print( paste( Files_All_vector_g_parents[i],   ":", sep="") )
  print( getCoverageStats( myobj_g_parents[[i]] )  )
}
sink()



pdf( file=paste(myOutDir_sub1_g_parents, "11A_MethylationStats-kept.pdf", sep="/")  )
for( i in c(1:length(filtered.myobj_g_parents)) ) {
  getMethylationStats(filtered.myobj_g_parents[[i]], plot=TRUE, both.strands=FALSE )
}
dev.off()
sink( file=paste(myOutDir_sub1_g_parents, "11B_MethylationStats-kept.txt", sep="/")  )
for( i in c(1:length(filtered.myobj_g_parents)) ) {
  print("##############################################")
  print( paste(Files_All_vector_g_parents[i],  ":", sep="") )
  print( getMethylationStats( filtered.myobj_g_parents[[i]] )  )
}
sink()
pdf( file=paste(myOutDir_sub1_g_parents, "12A_CoverageStats-kept.pdf", sep="/")  )
for( i in c(1:length(filtered.myobj_g_parents)) ) {
  getCoverageStats(filtered.myobj_g_parents[[i]], plot=TRUE, both.strands=FALSE )
}
dev.off()
sink( file=paste(myOutDir_sub1_g_parents, "12B_CoverageStats-kept.txt", sep="/")  )
for( i in c(1:length(filtered.myobj_g_parents)) ) {
  print("##############################################")
  print( paste( Files_All_vector_g_parents[i],   ":", sep="") )
  print( getCoverageStats( filtered.myobj_g_parents[[i]] )  )
}
sink()
...
pdf( file=paste(myOutDir_sub1_g_parents, "13A_MethylationStats-removed.pdf", sep="/")  )
for( i in c(1:length(removed.myobj_g_parents)) ) {
  getMethylationStats(removed.myobj_g_parents[[i]], plot=TRUE, both.strands=FALSE )
}
dev.off()
sink( file=paste(myOutDir_sub1_g_parents, "13B_MethylationStats-removed.txt", sep="/")  )
for( i in c(1:length(removed.myobj_g_parents)) ) {
  print("##############################################")
  print( paste(Files_All_vector_g_parents[i],  ":", sep="") )
  print( getMethylationStats( removed.myobj_g_parents[[i]] )  )
}
sink()
pdf( file=paste(myOutDir_sub1_g_parents, "14A_CoverageStats-removed.pdf", sep="/")  )
for( i in c(1:length(removed.myobj_g_parents)) ) {
  getCoverageStats(removed.myobj_g_parents[[i]], plot=TRUE, both.strands=FALSE )
}
dev.off()
sink( file=paste(myOutDir_sub1_g_parents, "14B_CoverageStats-removed.txt", sep="/")  )
for( i in c(1:length(removed.myobj_g_parents)) ) {
  print("##############################################")
  print( paste( Files_All_vector_g_parents[i],   ":", sep="") )
  print( getCoverageStats( removed.myobj_g_parents[[i]] )  )
}
sink()
...
##################################################################################################################
{
  ...
  ###
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/Results_100bp_Bin/1-matrix-all",  sep="") ,   
                       binSize_temp1 = 100,   binBases_temp1 = 1, dataFrame_temp1 = dataFrame_temp100_A  )
}
##################################################################################################################
...
##################################################################################################################
{
  ...
  ###
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/1_NC-vs-ART_1bp_1bases",  sep="") ,   
                       binSize_temp1 = 1,   binBases_temp1 = 1, dataFrame_temp1 = dataFrame_temp1_A  )
}
##################################################################################################################
...
##################################################################################################################
{
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/2_NC-vs-IVFfresh_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/3_NC-vs-IVFfrozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/4_NC-vs-ICSIfresh_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/5_NC-vs-ICSIfrozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/6_IVF_fresh_vs_IVF_frozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/7_IVF_fresh_vs_ICSI_fresh_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/8_IVF_fresh_vs_ICSI_frozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/9_IVF_frozen_vs_ICSI_fresh_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/10_IVF_frozen_vs_ICSI_frozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/11_ICSI_fresh_vs_ICSI_frozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/12_NC_vs_IVF_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/13_NC_vs_ICSI_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/14_NC_vs_fresh_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/15_NC_vs_frozen_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  ###
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/16_NC_vs_ART_1kb_3bases",  sep="") ,   
...
##################################################################################################################
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/17_IVF_vs_ICSI_1kb_3bases",  sep="") ,   
...
##################################################################################################################
{
  ...
  myMainFunction_1_g(  myobj_temp1 = myobj_nor_g,   path_temp1 = paste(outDir_g, "/18_fresh_vs_frozen_1kb_3bases",  sep="") ,   
